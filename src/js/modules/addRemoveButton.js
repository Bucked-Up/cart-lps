...
const addRemoveButton = ({ card, product }) => {
  ...
  button.addEventListener("click", () => {
    const qttyInput = card.querySelector("[qtty-input]");
    updateDomQuantities(-qttyInput?.value || -1);
    updatePrices(product, "subtract", qttyInput?.value || 1);
    card.remove();
    removeProduct({ productId: product.id });
    if (Object.keys(getProducts()).length < 1) closeCart();
  });
